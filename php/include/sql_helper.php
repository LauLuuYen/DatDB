...
class SQL_Helper {
    ...
    */
    ...
    public function getAllAssessments() {
        $stmt = $this->conn->prepare("SELECT groupid, reportid, current_status, feedback, score, userid, timestamp FROM assessments A JOIN status S ON A.statusid = S.id;");
        
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($groupid, $reportid, $status, $feedback, $score, $userid, $timestamp);
            $data = array();
    ...
    */
	...
	{
		$stmt = $this->conn->prepare("SELECT groups.name,reports.groupid, reportid, AVG(score) FROM assessments INNER JOIN reports ON (assessments.reportid = reports.id) INNER JOIN groups ON (reports.groupid = groups.id) GROUP BY groupid ORDER BY AVG(score) DESC;");
		if ($stmt->execute()) 
		{
	            $stmt->store_result();
